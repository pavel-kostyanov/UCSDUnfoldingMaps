...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		if (offline) {
		    map = new UnfoldingMap(this, 200, 50, 850, 800, new MBTilesMapProvider(mbTilesString));
		    earthquakesURL = "2.5_week.atom";  // The same feed, but saved August 7, 2015
		}
		else {
			map = new UnfoldingMap(this, 200, 50, 850, 800, new EsriProvider.WorldPhysical()) ;
	...
	private void printQuakes() 
	{
		...
		for (Marker cm : countryMarkers) { 
			...
			for(Marker qm : quakeMarkers) {
				if((String)cm.getProperty("name") == (String)qm.getProperty("country") & ((EarthquakeMarker)qm).isOnLand()) {
					quakeCounter++; 
				}
			}
			if(quakeCounter != 0) {
		...
		//     String name = (String)cm.getPropery("name");
